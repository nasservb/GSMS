...
class picture 
{
	...
	public function getPicture($id)
	{
		...
		{
			$row='';$max=0;
			GSMS::$class['DB']->run('select * from `picture` where `id`='.$id,
							'picture.php',	//file of code
							$row,			//variable to return first row			
							$max,			//variable to return row count	
	...
	public function deletePicture($id)
	{
		...
		if ($id>0)
		{
			...
			GSMS::$class['DB']->run('delete   from `Picture` where `id`='.$id,
							'Picture.php',	//file of code
							$row,			//variable to return first row			
							$max,			//variable to return row count	
							'select requested Picture'	//sql code subject	
	...
	public function listPictureByItemType(
					$itemType,
					$begin=0,
					$end=0)
	{
	...
	public function listPictureByItem(
					$item,
					$itemType,
					$begin=0,
					$end=0)
	...
	public function listPictureByUser(
					$user,
					$begin=0,
					$end=0)
	{
	...
	public function searchPictures(
					$Title='',
					$BeginCreateDate='',
					$EndCreateDate='',
					$ItemId=0,
					...
					$end=0)
	{
		...
		$q_count = GSMS::$class['DB']->run('select count(*)as cnt from `picture` '.$q , 
									'picture.php',	//file of code
									$row,			//variable to return first row			
									$max,			//variable to return row count	
									'list picture '	//sql code subject	
		...
		$query=GSMS::$class['DB']->run('select * from `picture`  '.$q.' limit '.$begin.','.$end, 
								'picture.php',	//file of code
								$row,			//variable to return first row			
								$max,			//variable to return row count	
								'list picture '	//sql code subject	
		...
		if($result_count>0)
		{
			...
			return array($tempPictures,$begin,$result_count);
		}else
		{
			return 404; // no result for query
		}//else
	...
	public function review($sql,$result_count , $begin=0,$end=30)
	{
		...
		$row=0;
		$max=0;
		$query=GSMS::$class['DB']->run($sql.' limit '.$begin.','.$end, 
								'picture.php',	//file of code
								$row,			//variable to return first row			
								$max,			//variable to return row count	
	...
	public function toString($id=0)
	{
		...
		{
			$row='';$max=0;
			GSMS::$class['DB']->run('select `title` from  `picture` where `id`='.$id,
							'picture.php',	//file of code
							$row,			//variable to return first row			
							$max,			//variable to return row count	
