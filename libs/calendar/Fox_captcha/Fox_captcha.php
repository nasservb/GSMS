<?php
	...
	class Fox_captcha
	{
		 ...
		 *               2- JPG will make the out put as Image. It is not required by the user to use it. It is only
		 *               set to be used by the classe's object internaly
		 */
		function make_it($state_type = 'HTML', $name_session = 'Fox_captcha_ss')
		{
			if ($state_type == 'HTML') {
				...
				$output .= "<img style=\"display:none\" src=\"" . $this->image_dir . 'fox_captcha_image_loading.gif' . "\" />";
			...
			";
				$output .= '<img align="middle" alt="Fox Captcha!" id="FoxCaptchaImage_' . $uniqu_id . '" src="' . $this->image_dir . 'fox_captcha_image.php"> &nbsp; <a href="javascript:FoxCaptchaReLoad_ss()">' . $this->en_reload . '</a>';
			return $output ;
			}
			elseif ($state_type == 'JPG') {
				...
				return('<b>Fox_captcha Error: Method make_it takes bad arguments.</b>');
			}
