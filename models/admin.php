...
class admin 
{
	...
	)
	{
		...
		if($max ==0 )
			return array(array(),$begin,$result_count);
		else 
	...
	public function getAllEmployers($begin=0, $end=30)
	{
		if(intval($begin)==0)
			$begin=0;
		...
		if($end> GSMS::$config['page_item_per_page'] )
			$end= $end  - $begin;
		$where = ' WHERE `admin_type`=1 or `admin_type`=3 '.' limit '.$begin.','.$end;
		GSMS::$class['DB']->run('select count(*)as cnt from  `admin` '.$where,
								'admin.php',	//file of code
								$row,			//variable to return first row			
								$max,			//variable to return row count	
								'searchAdmin count'	//sql code subject	
		...
		$tempAdminRows=GSMS::$class['DB']->run('SELECT * FROM `admin` '. $where ,
								'admin.php',	//file of code
								$row,			//variable to return first row			
								$max,			//variable to return row count	
								'searchAdmin '	//sql code subject	
