    ...
    var n = function () {
        function n(n, t) {
            ...
            i.el = $(n), u = i.el, i.options = $.extend(!1, {}, o, t), r = i.options, _fontSize = r.fontSize, _cw = parseInt(r.cellWidth), _ch = parseInt(r.cellHeight), i.cellStyle = "style='width:" + _cw + "px;height:" + _ch + "px;line-height:" + _ch + "px; font-size:" + _fontSize + "px; ' ", i.headerStyle = "style='height:" + _ch + "px;line-height:" + _ch + "px; font-size:" + (_fontSize + 4) + "px;' ", i.selectUlStyle = "style='margin-top:" + _ch + "px;height:" + (_ch * 7 + 20) + "px; font-size:" + (_fontSize - 2) + "px;' ", i.selectMonthLiStyle = "style='height:" + (_ch * 7 + 7) / 4 + "px;line-height:" + (_ch * 7 + 7) / 4 + "px; width:" + (7 * _cw + 1) / 3 + "px;width:" + 7 * _cw / 3 + "px\\9;' ", i.selectYearLiStyle = "style='height:" + (_ch * 7 + 10) / 6 + "px;line-height:" + (_ch * 7 + 10) / 6 + "px; width:" + (7 * _cw - 14) / 3 + "px;width:" + (7 * _cw - 15) / 3 + "px\\9;' ", i.footerStyle = "style='height:" + _ch + "px;line-height:" + _ch + "px; font-size:" + _fontSize + "px;' ", i.jDateFunctions = new jDateFunctions, i.options.selectedDate == undefined && (e = new RegExp("^([1-9][0-9][0-9][0-9])/([0]?[1-9]|[1][0-2])/([0]?[1-9]|[1-2][0-9]|[3][0-1])$"), u.is("input") ? e.test(u.val()) && (i.options.selectedDate = u.val()) : e.test(u.html()) && (i.options.selectedDate = u.html())), i._persianDate = i.options.selectedDate != undefined ? (new persianDate).parse(i.options.selectedDate) : i.now(), r.selectableYears != undefined && r.selectableYears._indexOf(i._persianDate.year) == -1 && (i._persianDate.year = r.selectableYears[0]), i.options.selectableMonths._indexOf(i._persianDate.month) == -1 && (i._persianDate.month = r.selectableMonths[0]), i.persianDate = i._persianDate, i._id = "pdp-" + Math.round(Math.random() * 1e7), i.persianDate.formatDate = r.formatDate, i.calendar = $('<div id="' + i._id + '" class="pdp-' + r.theme + '" />'), (u.attr("pdp-id") || "").length || u.attr("pdp-id", i._id), u.addClass("pdp-el").bind("click",function (n) {
                ...
                var n = u.offset();
                i.calendar.css({top: n.top + u.outerHeight() + r.calendarPosition.y + "px", left: n.left + r.calendarPosition.x + "px"})
            }, i.onresize = f, $(window).resize(f), $("body").append(i.calendar), i.render(), f()
        }
        ...
        return n.prototype = {show: function () {
            ...
            var n = this, e, r, h;
            _monthYear = $('<div class="" />'), _monthYear.appendTo(this.calendar), _head = $('<div class="header" ' + n.headerStyle + " />"), _head.appendTo(this.calendar), _next = $('<div class="nextArrow" />').html(this.options.nextArrow).attr("title", "ماه بعد").bind("click", function () {
            ...
            var u = $('<ul class="monthSelect" ' + n.selectUlStyle + " />").hide(), t = $('<ul class="yearSelect" ' + n.selectUlStyle + " />").hide(), o = $("<span/>").html(n.options.months[n.persianDate.month - 1]).mousedown(function () {
                ...
                $.each(n.options.selectableYears || (f ? e.reverse() : e), function (i, r) {
                    ...
                    r == n.persianDate.year && u.addClass("selected"), u.attr("value", r), u.bind("click", function () {
                        n.persianDate.date = 1, n.persianDate.year = parseInt(r), n.render()
                    }), f ? t.prepend(u) : t.append(u)
                })
            };
            for (f(), i = 1; i <= 12; i++)e = n.options.months[i - 1], r = n.options.selectableMonths._indexOf(i) == -1 ? $('<li class="disableMonth" ' + n.selectMonthLiStyle + " />").html(e) : $("<li " + n.selectMonthLiStyle + " />").html(e), i == n.persianDate.month && r.addClass("selected"), r.data("month", {month: e, monthNum: i}), r.hasClass("disableMonth") || r.bind("click", function () {
            ...
            }), _monthYear.append(u).append(t), h = $('<div class="monthYear" />').append(o).append("<span>&nbsp;&nbsp;<\/span>").append(s), _head.append(h), _prev = $('<div class="prevArrow" />').html(this.options.prevArrow).attr("title", "ماه قبل").bind("click", function () {
            ...
            for (_days = $('<div class="days" />'), _days.appendTo(this.calendar), jd = n.persianDate, jd.date = 1, _start = n.jDateFunctions.getWeekday(n.jDateFunctions.getJulianDayFromPersian(jd)), _end = n.jDateFunctions.getLastDayOfPersianMonth(n.persianDate), r = 0, t = 0; r < 6; r++) {
                for (_row = $("<div />"), i = 0; i < 7; i++, t++)t < _start || t - _start + 1 > _end ? _cell = $('<div class="nul cell " ' + n.cellStyle + " />").html("&nbsp;") : (_dt = n.getDate(n.persianDate, t - _start + 1), _today = "", _selday = "", n.now().compare(_dt) == 0 && (_today = "today"), n.options.selectedDate != undefined ? n.persianDate.parse(n.options.selectedDate).date == t - _start + 1 && (_selday = "selday") : t - _start + 1 == n.now().date && (_selday = "selday"), _fri = i == 6 ? "friday" : "", _cell = $('<div class="day cell ' + _fri + " " + _today + " " + _selday + '" ' + n.cellStyle + " />"), _cell.data("date", {jDate: _dt.toString("YYYY/MM/DD/DW"), gDate: n.jDateFunctions.getGDate(_dt)._toString(n.options.formatDate)}), _cell.html(n.options.persianNumbers ? n.jDateFunctions.toPersianNums(t - _start + 1) : t - _start + 1).bind("click", function () {
            ...
            var n = this;
            _footer = $('<div class="footer" ' + n.footerStyle + " />"), _footer.appendTo(this.calendar), n.options.selectableMonths._indexOf(n.persianDate.month) > -1 && (_goToday = $('<a class="goToday" />'), _goToday.data("date", {jDate: n.now().toString("YYYY/MM/DD/DW"), gDate: n.jDateFunctions.getGDate(n.now())._toString(n.options.formatDate)}), _goToday.attr("href", "javascript:;").html("هم اکنون").bind("click", function () {
            ...
            var i = this;
            n.is("input") ? n.val(i.persianDate.parse(t).toString(i.options.formatDate)) : n.html(i.persianDate.parse(t).toString(i.options.formatDate)), n.data("date", {jDate: t, gDate: this.jDateFunctions.getGDate(this.persianDate)._toString("YYYY/MM/DD/DW")}), this.options.onSelect()
...
var persianDate = function () {
    ...
    return n.prototype = {addMonth: function (n) {
    ...
    return n.prototype = {toPersianNums: function (n) {
