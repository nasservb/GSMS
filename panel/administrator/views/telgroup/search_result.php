...
class search_result
{
    ...
    function search_result($tempGroupArray)
    {
        ...
        $inf = array('page_title' => 'نتایج جستجو ');
		...
		$body = '<a class="btn btn-primary" href="' . GSMS::$class['template']->info['admin_url'] . 'telgroups/search">جستجوی پیشرفته</a><br/>';

        
        if (count($tempGroup) == 0) {
            $body .= '<div class="alert alert-warning"> موردی یافت نشد</div><a class="back_btn" href="'
                . GSMS::$class['template']->info['admin_url'] . '">برگشت</a>';
            $inf = array('title' => 'نتایج جستجو ', 'body' => $body, 'dir' => 'ltr');
		...
		$body .= GSMS::$class['template']->paging(
            GSMS::$class['template']->info['admin_url'] .  'telgroups/search_result/', $begin, $itemCount);
         ...
         $body .= ' </div>
		  ...
		  ';
			...
			$inf = array('title' =>'نتایج جستجو ', 'body' => $body);
			GSMS::$class['template']->index($inf);
			GSMS::$class['template']->footer($inf);
	}
