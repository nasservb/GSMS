...
class viewFacture
{
    function viewFacture($tempFacture)
    {
		...
		if($facture->isaccept == 1){
			$verifiySt= '
			<tr><td>تایید کننده</td><td>'.$tempFacture['acceptor']->name. ' ' . $tempFacture['acceptor']->family.'</td></tr>
			<tr><td>نام کاربری تایید کننده</td><td>'.$tempFacture['acceptor']->username.'</td></tr>';
		}
		...
		<tr><td><img id="imgicon" style="max-width:200px" src="'. GSMS::$siteURL.GSMS::$outputDir .'assets/images/biglogo.png"/></td><td>';
		if( intval($facture->totalAmount)-intval($facture->recieve)>0)
		{
			$body .= '<div><h4 class="text-danger" >پرداخت نشده</h4></div>
			
			<a class="btn btn-success" href="'.GSMS::$class['template']->info['user_url'].'accounting/buyOnline/'.$facture->id.'">پرداخت آنلاین  </a>
			';
		}
		else{
			$body .= '<div><h4 class="text-success" >پرداخت شده</h4></div><br/>
		...
		<tr><td>نام صاحب حساب :</td><td>طاقتی</td></tr>
		...
		<tr><td>مبلغ کل </td><td>'.number_format($facture->totalAmount).' ريال</td></tr>
		<tr><td>مبلغ پرداخت شده </td><td>'.number_format($facture->recieve).' ريال</td></tr>
		...
		else
		{
			...
			for($i=0 ; $i<$manTransactions[2] ; $i++){
				...
				<td><a  class="btn btn-primary" href=\''.GSMS::$class['template']->info['user_url'].'accounting/viewReceipt/' 
			...
			for($i=0 ; $i<$internetTransactions[2] ; $i++){
				...
				<td>'.number_format($internetTransactions[0][$i]->totalAmount).' ريال</td>
				<td>'.$internetTransactions[0][$i]->refNum.'</td>
				</tr>';
			}
