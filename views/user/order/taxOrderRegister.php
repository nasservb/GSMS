<?php
class taxOrderRegister
{
	function taxOrderRegister($info)
	{		
        $inf = array(
            'page_title' => 'سفارش چاپ فرم مالیاتی  ','activeTab'=>'orders'
        );
		...
		<input type="hidden" name="is_edit" value="'.(is_object($userOrder)? '1':'0').'" />
...
<table class="table table-hover">
	...
	';
		...
		<tr>';
			...
			if (count($info['orderServices'])>0 )
			{
				$body .='<td valign="top" style="width:150px" >خدمات کار تکمیلی:</td>
        				<td>
	        ...
	        (is_object($userOrder) && $userOrder->ostanId == $categorys[$i]['id'] ? ' selected' : '').' >' .
...
if (is_object($userOrder))
{
	...
	document.addEventListener("DOMContentLoaded", function(event) { 
			...
			var pic_array = '. json_encode($inf['orderPictures'] ) . ';	

			getShar($("#ostan").val());
	});
 
			var shahrId = '.intval($userOrder->shahrId).';
