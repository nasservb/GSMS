...
class view_comment
{
    ...
    function view_comment($tempComment)
    {
        ...
        if ( !is_object($tempComment)) {
            $body = ' نظری یافت نشد</table><br><a class="back_btn" href="'
                . GSMS::$class['template']->info['user_url'] . 'settings/comments">برگشت</a>';
	...
	<tr>	
		...
		<td>عملیات</td>	
		<td>
			<a class="btn btn-success" href="'.GSMS::$class['template']->info['user_url'] . 'settings/commentAccept/'.
				$tempComment->id . '">
				پذیرش 
			</a>
			<a class="btn btn-danger" href="'.GSMS::$class['template']->info['user_url'] . 'settings/commentDelete/'.
							$tempComment->id . '">
				حذف
			</a>
		</td>
